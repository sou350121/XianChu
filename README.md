...
- **艺术声明 / 展演动机 / 三幕脚本**：[ARTISTIC_STATEMENT.md](./ARTISTIC_STATEMENT.md)

## 从“工程”开始（入口索引）

- **PRD（验收口径）**：[PRD.md](./PRD.md)
- **DESIGN（系统设计）**：[DESIGN.md](./DESIGN.md)
- **任务清单（按 M0–M3 执行）**：[TASK_LIST.md](./TASK_LIST.md)
- **团队协作（角色/实习生配置）**：[TEAM.md](./TEAM.md)
- **技术路线与决策原则**：[AGENT_robotExpert.md](./AGENT_robotExpert.md)
- **PRD 对齐计划（索引）**：[PRD_PLAN.md](./PRD_PLAN.md)
- **工程 runbooks（构建/启动/录制/评测/现场安全）**：[docs/README.md](./docs/README.md)
...
最后，**可复盘**是把研究变成工程迭代的必要条件：我们将 SR/IR/ER 与失败分布写进 [PRD.md](./PRD.md)，把“每次翻车到底是哪类接触相位问题”变成可定位、可对比、可回归的指标。更完整的一手论文/综述回链仍以 [DESIGN.md](./DESIGN.md) 文末 References 为准。

</details>

## 工程目录（你应该从这里开始）

- **ROS 2 Workspace**：[ws/](./ws/)
  - [ws/src/](./ws/src/)：ROS 2 packages（包边界就是物理导轨）

- **SSOT：契约层**：[contracts/](./contracts/)
  - [contracts/ros_interfaces/](./contracts/ros_interfaces/)：topic/service/action 规范（人类可读）
  - [contracts/frames/](./contracts/frames/)：坐标系与 tf 树规范（人类可读）
  - [contracts/data_schema/](./contracts/data_schema/)：数据/episode schema 与时间同步规范
  - [contracts/safety/](./contracts/safety/)：限位/速度/力矩/急停策略（默认约束）

- **SSOT：配置层**：[configs/](./configs/)
  - [configs/hardware/](./configs/hardware/)：硬件与序列号绑定配置（driver 参数）
  - [configs/control/](./configs/control/)：控制器与限幅配置
  - [configs/demo/](./configs/demo/)：demo profile（场地/琴/弓/曲目）配置

- **SSOT：校准层**：[calibration/](./calibration/)
- **工程文档（runbooks）**：[docs/](./docs/)（入口：[docs/README.md](./docs/README.md)）
- **工具与脚本**：[tools/](./tools/)
- **数据注册表（pointers-only）**：[data_registry/](./data_registry/)（只存外部位置/版本/统计，不存大文件）

## SSOT 规则（最小但强约束）

- **物理导轨**：新增功能必须放进正确的 ROS2 package；禁止“到处扔脚本”。
  - packages 的依赖关系以 [contracts/ros_interfaces/](./contracts/ros_interfaces/) 与包职责为准。
- **契约优先（contract-first）**：先改 [contracts/](./contracts/) 再改实现；实现必须对齐契约。
- **pointers-only**：视频/音频/触觉原始流/模型权重不入库，只登记在 [data_registry/](./data_registry/)。
